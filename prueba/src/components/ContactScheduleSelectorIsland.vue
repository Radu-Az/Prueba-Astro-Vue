<template>
  ...
  <div ref="scheduleRootRef" class="mt-5">
    <p class="text-sm font-semibold text-slate-700">{{ copy.title }}</p>
    <p class="mt-2 text-sm text-slate-600">{{ copy.description }}</p>
    ...
    <div class="relative z-40 mt-5 inline-block max-w-full">
      <button
        ...
        class="inline-flex items-center justify-center gap-2 rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-semibold text-slate-700 transition hover:border-slate-400 hover:text-slate-900"
    ...
    <p
      ...
      aria-live="polite"
      class="mt-5 rounded-xl border border-cyan-200 bg-cyan-50 px-4 py-3 text-sm text-slate-700"
    >
